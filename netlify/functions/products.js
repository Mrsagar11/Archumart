...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    switch (event.httpMethod) {
      case 'GET': {
        // Retrieve single product or list
        ...
        // List all products. Seed if empty.
      ...
      case 'POST': {
        ...
        if (!data.name || !data.price || !data.category) {
          return { statusCode: 400, headers, body: JSON.stringify({ error: 'Name, price, and category are required' }) };
        }

        // Determine new unique numeric ID
      ...
      case 'PUT': {
        ...
        if (isNaN(productId)) {
          return { statusCode: 400, headers, body: JSON.stringify({ error: 'Product ID is required for update' }) };
        }
        ...
        return { statusCode: 200, headers, body: JSON.stringify({ success: true, product: { id: productId, ...updateData } }) };
      }
      ...
      case 'DELETE': {
        ...
        if (!idParam) {
          return { statusCode: 400, headers, body: JSON.stringify({ error: 'Product ID parameter is required' }) };
        }
        ...
        return { statusCode: 200, headers, body: JSON.stringify({ success: true, message: 'Product deleted successfully' }) };
      }
